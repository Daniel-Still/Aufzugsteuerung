...
int dritter_Stock_von_oben_state = 1;
int get_dritter_Stock_von_oben_state()
{
    return dritter_Stock_von_oben_state;
}
...
void func_dritter_Stock_von_oben(void)
{
    switch(dritter_Stock_von_oben_state)
    {
        case check_Haltebedingung:
            if((Stockwerk_gerufen[3] == 1) || (Notrufbetrieb == 1))
            {
                dritter_Stock_von_oben_state = warte_auf_Position;
            }
            else if( (stepper1.currentPosition() - Pos_Stockwerk_3  ) < max_brems_steps) 
            {
                stepper1.move(Pos_Stockwerk_2 - stepper1.currentPosition() );
                Hauptfunktionen_state = zweiter_Stock_von_oben;
                dritter_Stock_von_oben_state = check_Haltebedingung;
            }
        ...
        case warte_auf_Position:
            ...
            {
                dritter_Stock_von_oben_state = Tuer_oeffnen;
            }
            break;
        ...
        case Tuer_oeffnen:
            ...
            if(Timer_Tuer_oeffnen.TIMER_ALARM == 1)
            {
                ...
                if(Notrufbetrieb == 0)
                {
                    dritter_Stock_von_oben_state = Tuer_schliessen; 
                }
        ...
        case Tuer_schliessen:
            ...
            if(Timer_Tuer_schliessen.TIMER_ALARM == 1)
            {
                ...
                dritter_Stock_von_oben_state = check_Anforderung;            
            }
